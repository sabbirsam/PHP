...
if('decode' == $task){
    ...
    if($data != ''){
        $encodedScrambleData = decodedScrambleData($data , $key);
    }
}
...
//decode code
function decodedScrambleData($originalData, $key){
    $original_key = 'abcdefghijklmnopqrstuvwxyz1234567890';
    ...
    for($i=0; $i<$length; $i++){
        ...
        $postion= strpos($key, $currentCharacter);//key used

        if($postion !== false){
            $data .= $original_key[$postion];   //here $original key ued
        }
        else{
            $data .= $currentCharacter;
        }
